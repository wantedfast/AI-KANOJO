...
describe("ElevenAgents renderer adapter", () => {
  ...
  it("does not let an unconfirmed Agent transcript restart Thinking after standalone V3 playback", async () => {
    ...
    captionCallbacks.onPartial("real question");
    getCallbacks().onMessage({ role: "user", message: "real question" });
    getCallbacks().onMessage({ role: "agent", message: "standalone reply" });
    await vi.waitFor(() => expect(adapter.getSnapshot().phase).toBe("completed"));
    const completedTurnCount = adapter.getSnapshot().voiceTurns.length;

    getCallbacks().onVadScore({ vadScore: 0.8 });
    getCallbacks().onMessage({ role: "user", message: "You want to learn how to say?" });

    expect(adapter.getSnapshot()).toMatchObject({ phase: "completed", activeTurnId: "" });
    expect(adapter.getSnapshot().voiceTurns).toHaveLength(completedTurnCount);
    expect(adapter.getSnapshot().messages.map((message) => message.content)).not.toContain("You want to learn how to say?");
  });

  it("accepts a standalone V3 user turn when the local caption sidecar confirms real speech", async () => {
    vi.stubGlobal("MediaStream", class {
      constructor(tracks) { this.tracks = tracks; }
      getTracks() { return this.tracks; }
    });
    ...
    getCallbacks().onMessage({ role: "user", message: "confirmed question" });
    expect(adapter.getSnapshot().phase).toBe("listening");
    expect(adapter.getSnapshot().messages).toHaveLength(0);

    captionCallbacks.onPartial("confirmed question");
    expect(adapter.getSnapshot()).toMatchObject({ phase: "thinking", transcript: "confirmed question" });
    expect(adapter.getSnapshot().messages.map((message) => message.content)).toContain("confirmed question");
  });

  it("falls back to the Agent transcript if the standalone V3 caption sidecar fails", async () => {
    vi.stubGlobal("MediaStream", class {
      constructor(tracks) { this.tracks = tracks; }
      getTracks() { return this.tracks; }
    });
    ...
    getCallbacks().onMessage({ role: "user", message: "fallback question" });
    expect(adapter.getSnapshot().phase).toBe("listening");

    captionCallbacks.onError("caption unavailable");
    expect(adapter.getSnapshot()).toMatchObject({ phase: "thinking", transcript: "fallback question" });
    expect(adapter.getSnapshot().messages.map((message) => message.content)).toContain("fallback question");
  });
